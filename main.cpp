...
int main(int argc, char *argv[]) {
    ...
    if (command == "add") {
		// usage
        if (argc < 4) {
            cout << "Usage: add <list_name> <task_name> [description] [location] [subject] [priority]" << endl;
            cout << "Priority: 0=low, 1=medium, 2=high" << endl;
            return 1;
        }
        ...
        string description = (argc > 4) ? argv[4] : "";
        string location = (argc > 5) ? argv[5] : "";
        string subject = (argc > 6) ? argv[6] : "";
        Priority priority = Priority::medium;
        if (argc > 7) {
            int p = stoi(argv[7]);
			// if valid then set the priority
            if (p >= 0 && p <= 2) {
                priority = static_cast<Priority>(p);
            }
        }
        
        util::addTask(listName, taskName, description, location, subject, priority);
